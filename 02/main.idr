...
doCommands : (Int, Int, Int) -> Command -> (Int, Int, Int)
doCommands (p, d, a) (Forward x) = (p + x, d + a * x, a)
doCommands (p, d, a) (Up x) = (p, d, a - x)
doCommands (p, d, a) (Down x) = (p, d, a + x)

run : String -> IO ()
run s = do let (p, d, _) = foldl doCommands (0, 0, 0) $ parseLines parseCommand s
           putStrLn $ show $ p * d
